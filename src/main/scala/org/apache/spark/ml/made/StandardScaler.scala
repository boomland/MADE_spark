...
class StandardScaler(override val uid: String) extends Estimator[StandardScalerModel] with StandardScalerParams with DefaultParamsWritable {
...
// companion object: 1 экземпляр класса, чтобы можно было читаться
